...
import { useCart } from "../../lib/CartContext"; // পাথ আপনার ফোল্ডার অনুযায়ী দিন
import LoginAlertModal from "../../components/LoginAlertModal"; // পাথ আপনার ফোল্ডার অনুযায়ী দিন
import { useAuth } from "../../context/AuthContext"; // পাথ আপনার ফোল্ডার অনুযায়ী দিন
...
const FeaturedSection = () => {
  ...
  const { user } = useAuth(); // Auth context থেকে ইউজার নিন
  const { addToCart, cart } = useCart(); // Cart context নিন
 ...
 const handleAddToCart = (product) => {
  ...
  addToCart(product); // এটি এখন পুরো অবজেক্ট স্টোর করবে
};

  // ফিচারড প্রোডাক্ট ফেচ করা
  useEffect(() => {
  ...
  return (
    <section className="py-20 px-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="text-3xl font-bold">Featured Releases</h2>
          </div>
                  ...
                  <button
                    ...
                    className={`p-4 rounded-xl transition-all shadow-lg active:scale-95 ${
                      cart.find((i) => i._id === product._id)
                        ? "bg-green-600 hover:bg-green-500" // কার্টে থাকলে কালার চেঞ্জ
      ...
      {/* --- View All Button (Shows only if products > 3) --- */}
      {filteredProducts.length > 3 && (
        <div className="mt-16 text-center">
          <button className="px-10 py-4 bg-white/5 border border-white/10 hover:border-purple-500/50 rounded-2xl font-bold uppercase text-xs tracking-[0.2em] transition-all hover:bg-purple-500/10">
