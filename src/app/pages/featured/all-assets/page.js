...
// ডেটা ফেচিং ফাংশন (ISR)
async function getInitialProducts() {
  const limit = 12;
  try {
    const res = await fetch(
      `https://uefn-maps-server.vercel.app/api/v1/products/featured?page=1&limit=${limit}`,
      {
        next: { revalidate: 60 }, // প্রতি ৬০ সেকেন্ডে ডেটা আপডেট হবে
      }
    );
...
export default async function AllAssetsPage() {
  ...
  return (
    <AllAssetsClient 
      ...
      initialTotalPages={initialData.meta?.totalPages || 1} 
    />
  );
}