    ...
    <div className="min-h-screen bg-[#0a0a0b] text-white pt-28 pb-20 px-6 font-sans">
        ...
        {activeVideo && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-5xl aspect-video bg-black rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(147,51,234,0.3)] border border-white/10"
            >
              <button
                onClick={() => setActiveVideo(null)}
                className="absolute top-4 right-4 z-10 p-2 bg-black/50 hover:bg-white/10 text-white rounded-full transition-all border border-white/10"
              >
                <X size={24} />
              </button>
            ...
            <div className="w-full lg:w-2/5 space-y-8">
              <div className="flex flex-wrap gap-2">
                {project.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] font-black text-purple-400 uppercase tracking-widest italic"
                  >
                    {tag}
                  </span>
                ))}
      ...
      <section className="py-40 max-w-7xl mx-auto text-center">
        <motion.div
          ...
          viewport={{ once: true }}
          className="bg-linear-to-b from-white/5 to-transparent border border-white/10 p-12 md:p-24 rounded-[4rem] relative overflow-hidden group"
        >
