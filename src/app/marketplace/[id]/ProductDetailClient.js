...
export default function ProductDetail({ initialProduct = null }) {
  ...
  const [activeImg, setActiveImg] = useState(initialProduct?.image?.url || "");
  const viewedProduct = useRef(null);
  useEffect(() => {
    function reportView() {
      if (product?._id && viewedProduct.current !== product._id && trackCommerce("view_item", [product])) {
        viewedProduct.current = product._id;
      }
    }
    reportView();
    window.addEventListener("uefnmap-analytics-ready", reportView);
    return () => window.removeEventListener("uefnmap-analytics-ready", reportView);
  }, [product]);
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="min-w-30 h-20 rounded-2xl bg-white/5 border border-white/5 shrink-0"
              />
            ))}
          </div>
