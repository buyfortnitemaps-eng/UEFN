...
const Topbar = ({ setIsMobileMenuOpen }) => {
  ...
  return (
    <header className="h-20 bg-[#0a0a0b]/60 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-6 sticky top-0 z-40">
      
      {/* Search & Mobile Toggle */}
      <div className="flex items-center gap-4">
        <button 
          onClick={() => setIsMobileMenuOpen(true)} 
          className="lg:hidden p-2.5 bg-white/5 rounded-xl text-gray-400 hover:text-white transition-colors"
        >
          <Menu size={22} />
        </button>
        ...
        <div className="hidden md:flex items-center bg-white/2 border border-white/5 px-4 py-2.5 rounded-2xl w-80 group focus-within:border-purple-500/50 transition-all">
          ...
          <input 
            ...
            className="bg-transparent border-none focus:ring-0 text-sm w-full ml-3 text-white placeholder:text-gray-600 font-medium" 
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-6">
        {/* Notifications */}
        <button className="hidden sm:flex p-2.5 bg-white/2 hover:bg-white/5 border border-white/5 rounded-xl text-gray-400 relative transition-all">
          <Bell size={20} />
        ...
        <div className="flex items-center gap-3 pl-6 border-l border-white/10 group cursor-pointer">
          <div className="text-right hidden sm:block">
            <p className="text-sm font-black text-white tracking-tight">{mongoUser?.name || "Admin"}</p>
          ...
          <div className="relative">
            <div className="w-11 h-11 rounded-2xl bg-linear-to-br from-purple-600 to-blue-600 p-0.5 shadow-lg shadow-purple-900/20 group-hover:rotate-6 transition-transform">
              <div className="w-full h-full bg-[#0d0d0f] rounded-[14px] overflow-hidden">
                <img 
          ...
          <ChevronDown size={16} className="text-gray-500 hidden sm:block group-hover:text-white transition-colors" />
        </div>
      </div>
    </header>
  );
